...
def quadratic():
    a_00 = choice([1, -1]);
    a_11 = randint(1,limit)/random_prime(limit);
    a_22 = randint(-1*limit,limit)/random_prime(limit);
    a_33 = randint(-1*limit,limit)/random_prime(limit);
    
    return a_00*(t - a_22)*(t - a_33);
    # return a_00*(a_11*t^2 + a_22*t + a_33);
    # return a_00*(t - a_11)*(t - a_22);
    
def complex_quadratic():
    a_00 = choice([1, -1]);
    c = randint(-1*limit,limit)/random_prime(limit);
    interval = ceil(sqrt(abs(c)));
    b = randint(-2*interval, 2*interval)
    return a_00*(x^2 + b*x + c)
...
def curve_full():
    ...
    c4 = f4.coefficients(sparse=False);
    c5 = f5.coefficients(sparse=False);
    c6 = f6.coefficients(sparse=False);
    
    M1 = matrix([[c1[2], c2[2], c3[2]], [c2[2], c4[2], c5[2]], [c3[2], c5[2], c6[2]]]);
    M2 = 1/2*matrix([[c1[1], c2[1], c3[1]], [c2[1], c4[1], c5[1]], [c3[1], c5[1], c6[1]]]);
    M3 = matrix([[c1[0], c2[0], c3[0]], [c2[0], c4[0], c5[0]], [c3[0], c5[0], c6[0]]]);
...
def curve():
    ...
    f1 = quadratic();
    f2 = quadratic();
    f3 = quadratic() if bool else complex_quadratic();
...
# This was originally from PSV and is now replaced with a blank function
def classify(f):
    classification = "N/A"
